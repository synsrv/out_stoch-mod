...
def synsrv_trace_figure(fit=True):
    ...
    fig, (ax1, ax2) = pl.subplots(1,2, gridspec_kw={'width_ratios': [1.3, 2]})
    fig.set_size_inches(5.2*0.5*2.2,3.6*0.5)
    ...
    if len(lts_df)>0:
        ...
        if fit:
            ...
            ax1.plot(xs, (1-29.2/10000)**xs, color='gray', alpha=0.5,
                    lw=2.5, label='exponential')
            ax2.plot(xs, (1-29.2/10000)**xs, color='gray', alpha=0.5,
                    lw=2.5, label='exponential')

        ax1.plot([0]+list(centers), [1.0]+list(srv),
                '.', label=label,
                markersize=10, markeredgewidth=0.5,
                markerfacecolor='None', color='#2c7fb8')

        ax2.plot([0]+list(centers), [1.0]+list(srv),
                '.', label=label,
                markersize=10, markeredgewidth=0.5,
                markerfacecolor='None', color='#2c7fb8')
